...
void task_conversion(void* p_params)
{
    ...
    for(;;)
    {
        ...
        */
        ...
        int16_t c_3 = ( pow((x1 - l1*sin(theta_1)),2) + pow(y1,2) + pow((0 - l1*cos(theta_1)), 2) - pow(l2, 2 ) - pow(l3, 2))/(2*l1*l2);
        int16_t s_3 = sqrt(1 - pow(c_3,2));
        int16_t k_1 = l2 + l3*c_3;
        int16_t k_2 = l3*s_3;
        int16_t theta_2_1_rad = atan2(sqrt( pow((x1 - l1*sin(theta_1)),2) + pow((0 - l1*cos(theta_1)), 2)), y1) - atan2(k_2, k_1) ; // [rad]
        int16_t theta_2_2_rad = atan2(sqrt( pow((x2 - l1*sin(theta_1)),2) + pow((0 - l1*cos(theta_1)), 2)), y2) - atan2(k_2, k_1) ; 
        //^^^ this needs a plus/minus in front of the swaure root, or an if statement and 2 equations?

        //THETA2 RAD TO DEGREES
        int16_t theta_2_1 = theta_2_1_rad*(180/3.1415); // [deg]
        int16_t theta_2_2 = theta_2_2_rad*(180/3.1415);

        // theta 2 will be plugged into an empiracally found eqn to get theta m
        int16_t theta_m1 = theta_2_1 * 1; // [deg] will fill in later. gear ratio? 
        int16_t theta_m2 = theta_2_2 * 1; // if gear ratio: Theta_m = Theta_2 * (radius of joint) / (radius of motor pulley)
